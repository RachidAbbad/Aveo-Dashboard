import { faThermometerHalf, faTint, faWater, faCog, faLevelDownAlt, faBullseye } from '@fortawesome/free-solid-svg-icons';
...
export class ChaudiereDashbordComponent  implements OnInit {
  ...
  constructor(private datepipe : DatePipe,private _cons : ConsommationService,private _comt : CompteurService,private siteService : SitesService, private rt: Router, private art: ActivatedRoute) 
  {
   ...
   void {
     ...
     switch(cliked.name) 
         {
           case "T° Départ" :
            window.open('https://admin.viatoile.com/#/clients/chaudiere/'+this.id+'/brute/td',);
           break;
          case "T° Retour" : 
          window.open('https://admin.viatoile.com/#/clients/chaudiere/'+this.id+'/brute/tr', '_blank');
           break;
           case "T° Fumées" : 
          window.open('https://admin.viatoile.com/#/clients/chaudiere/'+this.id+'/brute/tf', '_blank');;
           break;
           case "T° Inertie" : 
          window.open('https://admin.viatoile.com/#/clients/chaudiere/'+this.id+'/brute/ti', '_blank');;
           break;
           case "Dépression" : 
          window.open('https://admin.viatoile.com/#/clients/chaudiere/'+this.id+'/brute/de', '_blank');
           break;
           case "Luminosité" : 
          window.open('https://admin.viatoile.com/#/clients/chaudiere/'+this.id+'/brute/lu', '_blank');
           break;
        }
   }
  ...
  ngOnInit() 
  {
    ...
    this.ballon = {}
    this.silo.niveau = 78
    this.ballon.pression = 67;

    this.server = {}
    this.temperature = 24.76;

    this.taux = [{min:23.56, max:98.56},{min:45.26, max:65.89}]

    this.eau = {};
    this.title = "Capteur non détecter";
    this.live = "Erreur"
    ...
    this.chaudiere = {}
    this.chaudiere.id = "738GU9837"

    setInterval(() => {
      this.singleupdate.push(new Serie("T° Départ",Math.floor(Math.random() * (60 - 57 + 1)) + 57));
            this.singleupdate.push(new Serie("T° Retour",Math.floor(Math.random() * (190 - 187 + 1)) + 187));
            this.singleupdate.push(new Serie("T° Inertie",Math.floor(Math.random() * (98 - 95 + 1)) + 95));
            this.singleupdate.push(new Serie("T° Fumées",Math.floor(Math.random() * (78 - 76 + 1)) + 76));
            this.single=this.singleupdate;
            this.dataDepressionUpdate.push(new Serie("Dépression",Math.floor(Math.random() * (13 - 12 + 1)) + 12));
            this.dataDepression=this.dataDepressionUpdate;
            this.dataLumiereUpdate.push(new Serie("Luminosité",Math.floor(Math.random() * (99 - 98 + 1)) + 98));
            this.dataLumiere=this.dataLumiereUpdate;
    }, 1000*1.5);

    //---------------------/
    /*
        var idfuit = "600860f0d3628f52ed61ba00";


        this._comt.GetCompteurById("5fbccf26e06d8cb8a4ac500e").subscribe(result =>{
        ...
        this._comt.GetCompteurById(idfuit).subscribe(result =>{
        ...
        this.sub =  t.subscribe(t =>{
            ...
            this.getDataByCapteur("5fbccf26e06d8cb8a4ac500e",1);
            this.getserverbysite("600860a9d3628f52ed61b9ff");
            this.GetCurrentdata("61314a09252756ca36438401","61280dbcd34cfdd5ea0983f8","61280f8314665459bc9413d2");
  ...
  GetCurrentdata(idBallon,idsilo,idprod)
  {
      ...
      this._cons.GetLastEauBeat(id).subscribe(result => 
        {
           ...
           var last = new Date(result.data.timestamp);
            ...
            {
              this._cons.GetCurrentEauData(id,1).subscribe(
                result => 
                {
     ...
     //$('body .modals').remove();
      ...
      updatedata(data,interval)
      {
        ...
        for(let value of data)
        {
            ...
            this.singleupdate.push(new Serie("T° Inertie",value.temperatura_Inercia));
