...
export class NavComponent implements  OnInit{
  ...
  constructor( private ser: UserService, private notif_ser: NotificationService) {
  ...
  getNotification(){
    ...
    //--For Demo Purposes--//
    let index_current_notif = 0;
    const some_notifs = [
      {
        title: 'Alert Chaudière',
        body: 'La température de chadière avec id=8938 est élevée, sélectionnez un technicien pour le réparer.',
      },
      {
        title: 'Avertissement de sécurité',
        body: 'Un technicien non autorisé a accédé au site de production, vérifiez-le.',
      },
      {
        title: 'Avertissement de niveau de silo',
        body: "Le silo du site avec l'id 783 a atteint sa capacité maximale",
      },
    ];

    setInterval(() => {
      this.notifications.push(some_notifs[index_current_notif]);
      ++index_current_notif===3 ? index_current_notif = 0 : null;
    }, 1000*20);
    //---------------------//
